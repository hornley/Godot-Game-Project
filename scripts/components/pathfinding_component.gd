...
@export var bridges: TileMapLayer
	...
	for i in range(region.position.x, region.size.x + 1):
		for j in range(region.position.y, region.size.y + 1):
			...
			if cell in bridges_used_cells:
				continue
			var tile_data = land.get_cell_tile_data(cell)
			if tile_data == null or tile_data.get_custom_data("walkable") == false:
				solid_cells.append(cell)
	
	for house in houses.get_children():
		var house_offset: Vector2i = land.local_to_map(land.to_local(Vector2i(house.get_global_pos())))
		for cell in house.get_used_cells():
			solid_cells.append(cell + house_offset)
	
	for npc in npcs.get_children():
		var npc_cell_position: Vector2i = land.local_to_map(land.to_local(Vector2i(npc.global_position)))
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("right_click"):
		var mouse_pos: Vector2i = land.local_to_map(land.to_local(land.get_global_mouse_position()))
		path_find(mouse_pos)
...
func path_find(mouse_pos: Vector2i, include_last_path: bool = true) -> bool:
	...
	if path.is_empty():
		for cell_around in land.get_surrounding_cells(mouse_pos):
	...
	var positions: Array = path.map(func(cell): return land.map_to_local(cell))
	...
	right_click_animation_scene.global_position = land.map_to_local(mouse_pos)
...
func get_player_pos() -> Vector2i:
	var player_pos: Vector2i = land.local_to_map(land.to_local(PlayerManager.player.global_position))
	if astargrid2d.is_point_solid(player_pos):
		for cell_around in land.get_surrounding_cells(player_pos):
